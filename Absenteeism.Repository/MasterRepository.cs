...
namespace Absenteeism.Repository
{
    public class MasterRepository : IMasterRepository
    {
        ...
        public async Task UpdateNotification(int UserId)
        {
            try
            {
                using (IDbConnection con = new SqlConnection(_appSettings.ConnectionStrings.MyConnections))
                {
                    DynamicParameters dynamicParameters = new DynamicParameters();
                    dynamicParameters.Add(Constant.StoreProcedureAndParameters.ProcedureParameter_UserId, UserId);
                    await con.ExecuteAsync(Constant.StoreProcedureAndParameters.sp_UpdateNotification, param: dynamicParameters, commandTimeout: 3000, commandType: CommandType.StoredProcedure);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<tblNotification> InsertNotification(int UserId, int AvailabilityId)
        {
            try
            {
                ...
                using (IDbConnection con = new SqlConnection(_appSettings.ConnectionStrings.MyConnections))
                {
                    DynamicParameters dynamicParameters = new DynamicParameters();
                    dynamicParameters.Add(Constant.StoreProcedureAndParameters.ProcedureParameter_UserId, UserId);
                    dynamicParameters.Add(Constant.StoreProcedureAndParameters.ProcedureParameter_AvailabilityId, AvailabilityId);
